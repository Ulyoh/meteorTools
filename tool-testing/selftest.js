...
var expectThrows = markStack(function (f) {
  ...
  if (!threw) throw new TestFailure("expected-exception");
});
...
var execFileSync = function (binary, args, opts) {
  return Future.wrap(function (cb) {
    var cb2 = function (err, stdout, stderr) {
      cb(err, stdout);
    };
...
'meteor-base', 'mobile-experience', 'mongo', 'blaze-html-templates', 'session', 'jquery', 'tracker', "autopublish", "insecure", "standard-minifiers", "es5-shim"];

var setUpBuiltPackageTropohouse = function () {
  if (builtPackageTropohouseDir) return;
  builtPackageTropohouseDir = files.mkdtemp('built-package-tropohouse');

  if (config.getPackagesDirectoryName() !== 'packages') throw Error("running self-test with METEOR_PACKAGE_SERVER_URL set?");
...
var newSelfTestCatalog = function () {
  if (!files.inCheckout()) throw Error("Only can build packages from a checkout");
  ...
  var messages = buildmessage.capture({ title: "scanning local core packages" }, function () {
    ...
    selfTestCatalog.initialize({
      localPackageSearchDirs: [files.pathJoin(files.getCurrentToolsDir(), 'packages')]
    });
  });
  if (messages.hasMessages()) {
...
_.extend(Matcher.prototype, {
  ...
  match: function (pattern, timeout, strict) {
    var self = this;
    if (self.matchFuture) throw new Error("already have a match pending?");
    ...
    if (self.matchPattern instanceof RegExp) {
      ...
      if (m) {
        if (self.matchStrict && m.index !== 0) {
          ...
          f['throw'](new TestFailure('junk-before', { run: self.run, pattern: self.matchPattern }));
          return;
        }
        ret = m;
...
_.extend(OutputLog.prototype, {
  write: function (channel, text) {
    ...
    if (!_.has(self.buffers, 'channel')) self.buffers[channel] = { text: '', offset: 0 };
  ...
  forbid: function (pattern, channel) {
    ...
    _.each(self.lines, function (line) {
      if (channel && channel !== line.channel) return;

      var match = pattern instanceof RegExp ? line.text.match(pattern) : line.text.indexOf(pattern) !== -1;
      if (match) throw new TestFailure('forbidden-string-present', { run: self.run });
    });
  },
...
var Sandbox = function (options) {
  ...
  if (_.has(options, 'warehouse')) {
    if (!files.inCheckout()) throw Error("make only use a fake warehouse in a checkout");
  ...
  if (self.warehouse) self.execPath = files.pathJoin(self.warehouse, meteorScript);else self.execPath = files.pathJoin(files.getCurrentToolsDir(), meteorScript);
};
...
_.extend(Sandbox.prototype, {
  ...
  createApp: function (to, template, options) {
    ...
    if (options.dontPrepareApp) return;
  ...
  read: function (filename) {
    ...
    if (!files.exists(file)) return null;else return files.readFile(files.pathJoin(self.cwd, filename), 'utf8');
  },
  ...
  _makeEnv: function () {
    ...
    // latest release.
    if (!self.warehouse && release.current.isProperRelease()) env.METEOR_TEST_LATEST_RELEASE = release.current.name;
    return env;
  },
  ...
  _makeWarehouse: function (releases) {
    ...
    files.cp_r(files.pathJoin(builtPackageTropohouseDir, 'packages'), files.pathJoin(self.warehouse, packagesDirectoryName), { preserveSymlinks: true });
    ...
    tropohouseIsopackCache.eachBuiltIsopack(function (packageName, isopack) {
      ...
      if (!packageRec) throw Error("no package record for " + packageName);
      stubCatalog.collections.packages.push(packageRec);

      var versionRec = tropohouseLocalCatalog.getLatestVersion(packageName);
      if (!versionRec) throw Error("no version record for " + packageName);
    ...
    if (!toolPackageVersion) throw Error("no meteor-tool?");
...
_.extend(BrowserStackClient.prototype, {
  connect: function () {
    ...
    // memoize the key
    if (browserStackKey === null) browserStackKey = self._getBrowserStackKey();
    if (!browserStackKey) throw new Error("BrowserStack key not found. Ensure that you " + "have installed your S3 credentials.");
  ...
  _launchBrowserStackTunnel: function (callback) {
    ...
    '-skipCheck'];
    self.tunnelProcess = child_process.execFile('/bin/bash', ['-c', args.join(' ')]);

    // Called when the SSH tunnel is established.
    self.tunnelProcess.stdout.on('data', function (data) {
      if (data.toString().match(/You can now access your local server/)) callback();
    });
  }
});
...
_.extend(Run.prototype, {
  ...
  args: function () {
    ...
    if (self.proc) throw new Error("already started?");
  ...
  connectClient: function () {
    var self = this;
    if (!self.client) throw new Error("Must create Run with a client to use connectClient().");
  ...
  _exited: function (status) {
    ...
    if (self.exitStatus !== undefined) throw new Error("already exited?");
  ...
  _ensureStarted: function () {
    ...
    self.proc.on('close', function (code, signal) {
      if (self.exitStatus === undefined) self._exited({ code: code, signal: signal });
    });
    ...
    self.proc.on('error', function (err) {
      if (self.exitStatus === undefined) self._exited(null);
    });
  ...
  expectExit: markStack(function (code) {
    ...
    if (!self.exitStatus) throw new TestFailure('spawn-failure', { run: self });
  ...
  _killProcess: function () {
    if (!this.proc) throw new Error("Unexpected: `this.proc` undefined when calling _killProcess");
  ...
  tellMongo: markStack(function (command) {
    ...
    if (!self.fakeMongoPort) throw new Error("fakeMongo option on sandbox must be set");
    ...
    if (!self.fakeMongoConnection) {
      ...
      var lastStartTime = 0;
      for (var attempts = 0; !self.fakeMongoConnection && attempts < 100; attempts++) {
        ...
        (function () {
          ...
          var conn = net.connect(self.fakeMongoPort, function () {
            if (fut) fut['return'](true);
          });
          conn.setNoDelay();
          conn.on('error', function () {
            if (fut) fut['return'](false);
          });
          setTimeout(function () {
            if (fut) fut['return'](false); // 100ms connection timeout
          }, 100);

          // This is all arranged so that if a previous attempt
          // belatedly succeeds, somehow, we ignore it.
          if (fut.wait()) self.fakeMongoConnection = conn;
          fut = null;
        })();
      }

      if (!self.fakeMongoConnection) throw new TestFailure("mongo-not-running", { run: self });
    }
...
var getAllTests = function () {
  ...
  _.each(filenames, function (n) {
    if (!n.match(/^[^.].*\.js$/)) // ends in '.js', doesn't start with '.'
      return;
    try {
      if (fileBeingLoaded) throw new Error("called recursively?");
...
var readTestState = function () {
  ...
  var testState;
  if (files.exists(testStateFile)) testState = JSON.parse(files.readFile(testStateFile, 'utf8'));
  if (!testState || testState.version !== 1) testState = { version: 1, lastPassedHashes: {} };
  return testState;
};
...
var runTests = function (options) {
  ...
  if (testList.filteredTests.length === 0) {
    ...
    var disclaimers = '';
    if (testList.filteredTests.length < testList.allTests.length) disclaimers += " other";
