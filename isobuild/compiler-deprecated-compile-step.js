...
exports.makeCompileStep = function (sourceItem, file, inputSourceArch, options) {
  ...
  var compileStep = {
     ...
     */
    read: function (n) {
      if (n === undefined || readOffset + n > contents.length) n = contents.length - readOffset;
    ...
    addHtml: function (options) {
      if (!archinfo.matches(inputSourceArch.arch, "web")) throw new Error("Document sections can only be emitted to " + "web targets");
    ...
    addStylesheet: function (options) {
      if (!archinfo.matches(inputSourceArch.arch, "web")) throw new Error("Stylesheets can only be emitted to " + "web targets");
      if (typeof options.data !== "string") throw new Error("'data' option to addStylesheet must be a string");
      resources.push({
        type: "css",
        refreshable: true,
        data: new Buffer(files.convertToStandardLineEndings(options.data), 'utf8'),
    ...
    addJavaScript: function (options) {
      if (typeof options.data !== "string") throw new Error("'data' option to addJavaScript must be a string");
      if (typeof options.sourcePath !== "string") throw new Error("'sourcePath' option must be supplied to addJavaScript. Consider passing inputPath.");
