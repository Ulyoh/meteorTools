...
var _fibers2 = babelHelpers.interopRequireDefault(_fibers);
...
var LINKER_CACHE = new _lruCache2['default']({
  max: CACHE_SIZE,
  ...
  length: function (files) {
    return files.reduce(function (soFar, current) {
      return soFar + current.data.length + _utilsUtilsJs.sourceMapLength(current.sourceMap);
    }, 0);
  }
});
...
_.extend(exports.CompilerPluginProcessor.prototype, {
  runCompilerPlugins: function () {
    ...
    _.each(sourceProcessorsWithSlots, function (data, id) {
      ...
      Profile.time(jobTitle, function () {
...
var ResourceSlot = function (unibuildResourceInfo, sourceProcessor, packageSourceBatch) {
  var self = this;
...
_.extend(ResourceSlot.prototype, {
  addStylesheet: function (options) {
    var self = this;
    if (!self.sourceProcessor) throw Error("addStylesheet on non-source ResourceSlot?");
  ...
  addJavaScript: function (options) {
    ...
    if (!self.sourceProcessor && self.inputResource.extension !== "js") throw Error("addJavaScript on non-source ResourceSlot?");

    // By default, use the 'bare' option given to addFiles, but allow the option
    // passed to addJavaScript to override it.
    var bare = self.inputResource.fileOptions && self.inputResource.fileOptions.bare;
  ...
  addAsset: function (options) {
    var self = this;
    if (!self.sourceProcessor) throw Error("addAsset on non-source ResourceSlot?");
    ...
    self.outputResources.push({
      ...
      path: options.path,
      servePath: self.packageSourceBatch.unibuild.pkg._getServePath(options.path),
      hash: _fsWatchJs.sha1(options.data)
    });
  },
  addHtml: function (options) {
    var self = this;
    ...
    if (!archinfo.matches(unibuild.arch, "web")) throw new Error("Document sections can only be emitted to " + "web targets");
    if (options.section !== "head" && options.section !== "body") throw new Error("'section' must be 'head' or 'body'");
    if (typeof options.data !== "string") throw new Error("'data' option to appendDocument must be a string");
...
var PackageSourceBatch = function (unibuild, processor, _ref) {
...
_.extend(PackageSourceBatch.prototype, {
  ...
  _linkJS: Profile("PackageSourceBatch#_linkJS", function (jsResources) {
    ...
    var linkerOptions = {
      ...
      combinedServePath: isApp ? null : "/packages/" + colonConverter.convert(self.unibuild.pkg.name + (self.unibuild.kind === "main" ? "" : ":" + self.unibuild.kind) + ".js"),
    ...
    var cacheKey = _fsWatchJs.sha1(JSON.stringify({
