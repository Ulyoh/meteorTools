...
Isopack.convertIsopackFormat = Profile("Isopack.convertIsopackFormat", function (data, fromFormat, toFormat) {
  ...
  if (fromPos === -1) throw new Error("Can't convert from unknown Isopack format: " + fromFormat);
  if (toPos === -1) throw new Error("Can't convert to unknown Isopack format: " + toFormat);
...
_.extend(Isopack.prototype, {
  ...
  getSourceFilesUnderSourceRoot: Profile("Isopack#getSourceFilesUnderSourceRoot", function (sourceRoot) {
    ...
    var addSourceFilesFromWatchSet = function (watchSet) {
      _.each(watchSet.files, function (hash, filename) {
        ...
        if (relativePath.substr(0, 3) === '..' + files.pathSep) return;
  ...
  getUnibuildAtArch: Profile("Isopack#getUnibuildAtArch", function (arch) {
    ...
    var allowWrongPlatform = _ref.allowWrongPlatform;
  ...
  _checkPluginsInitialized: function () {
    var self = this;
    if (self._pluginsInitialized) return;
  ...
  _makePluginApi: function () {
    ...
    var Plugin = {
      ...
      _registerSourceProcessor: function (_ref2, factory, _ref3) {
        var extensions = _ref2.extensions;
        var filenames = _ref2.filenames;
        ...
        var featurePackage = _ref3.featurePackage;
  ...
  _loadUnibuildsFromPath: function (name, dir, options) {
    ...
    var _Isopack$readMetadataFromDirectory = Isopack.readMetadataFromDirectory(dir);

    var mainJson = _Isopack$readMetadataFromDirectory.metadata;
    ...
    if (options.isopackBuildInfoJson) {
      if (!options.firstIsopack) throw Error("can't merge isopacks with buildinfo");
    ...
    _.each(mainJson.builds, function (unibuildMeta) {
      ...
      if (unibuildJson.node_modules) {
        ...
        nodeModulesPath = files.pathJoin(unibuildBasePath, unibuildJson.node_modules);
      }
      ...
      _.each(unibuildJson.resources, function (resource) {
        ...
        } else throw new Error("bad resource type in isopack: " + JSON.stringify(resource.type));
      });
  ...
  saveToPath: Profile("Isopack#saveToPath", function (outputDir, options) {
    ...
    try {
      ...
      (function () {
        ...
        var writeLegacyBuilds = options.includePreCompilerPluginIsopackVersions && self.canWriteLegacyBuilds();
        ...
        _.each(self.unibuilds, function (unibuild) {
          ...
          if (unibuild.nodeModulesPath) {
            if (_.has(npmDirectories, unibuild.nodeModulesPath)) {
              ...
              nodeModulesPath = npmDirectories[unibuild.nodeModulesPath] = builder.generateFilename("npm/node_modules", { directory: true });
          ...
          _.each(unibuild.resources, function (resource) {
            if (_.contains(["head", "body"], resource.type)) {
              ...
              if (!(resource.data instanceof Buffer)) throw new Error("Resource data must be a Buffer");
          ...
          _.each(unibuild.resources, function (resource) {
            if (_.contains(["head", "body"], resource.type)) return; // already did this one
            ...
            unibuildJson.resources.push({
              type: resource.type,
              extension: resource.extension,
              file: builder.writeToGeneratedFilename(files.pathJoin(unibuildDir, resource.servePath || resource.path), { data: resource.data }),
              length: resource.data.length,
              offset: 0,
          ...
          _.each(unibuildInfos, function (unibuildInfo) {
            ...
            if (jsResourcesForLegacyPrelink.length === 1 && jsResourcesForLegacyPrelink[0].legacyPrelink) {
              ...
              _.each(unibuild.declaredExports, function (symbol) {
                if (_.has(packageVariableNames, symbol.name)) return;
              ...
              if (jsResourcesForLegacyPrelink.length) {
                ...
                _.each(results.assignedVariables, function (name) {
                  if (_.has(packageVariableNames, name)) return;
                  packageVariables.push({
                    name: name
                  });
  ...
  _writeTool: Profile("Isopack#_writeTool", function (builder) {
    ...
    pathsToTranspile.forEach(function (path) {
      ...
      var babelOptions = babel.getDefaultOptions(require('../tool-env/babel-features.js'));
  ...
  featureEnabled: function (featurePackageName) {
