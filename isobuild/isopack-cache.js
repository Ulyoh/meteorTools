...
_.extend(exports.IsopackCache.prototype, {
  buildLocalPackages: function (rootPackageNames) {
    ...
    if (self.cacheDir) files.mkdir_p(self.cacheDir);
  ...
  getIsopack: function (name) {
    var self = this;
    if (!_.has(self._isopacks, name)) throw Error("isopack " + name + " not yet loaded?");
  ...
  _ensurePackageLoaded: function (name, onStack) {
    var self = this;
    buildmessage.assertInCapture();
    if (_.has(self._isopacks, name)) return;
    ...
    if (!packageInfo) throw Error("Depend on unknown package " + name + "?");
  ...
  _lintLocalPackage: function (packageSource, isopack) {
  ...
  _checkUpToDate: function (isopackBuildInfoJson) {
    ...
    // up to date!
    if (!isopackBuildInfoJson) return false;
  ...
  _shouldLintPackage: function (packageSource) {
    if (this._lintLocalPackages) return true;
    if (!this._lintPackageWithSourceRoot) return false;
  ...
  getLintingMessagesForLocalPackages: function () {
    ...
    this._packageMap.eachPackage(function (name, packageInfo) {
      var isopack = _this._isopacks[name];
      if (packageInfo.kind === 'local') {
        if (!_this._shouldLintPackage(packageInfo.packageSource)) {
          return;
        }
