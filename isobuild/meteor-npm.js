...
cleanup.onExit(function () {
  _.each(tmpDirs, function (dir) {
    if (files.exists(dir)) files.rm_recursive(dir);
  });
});
...
meteorNpm.updateDependencies = function (packageName, packageNpmDir, npmDependencies, quiet) {
  ...
  try {
    ...
    if (files.exists(newPackageNpmDir)) files.rm_recursive(newPackageNpmDir);
...
meteorNpm.dependenciesArePortable = function (packageNpmDir) {
  ...
  var search = function (dir) {
    return _.find(files.readdir(dir), function (itemName) {
      if (itemName.match(/\.node$/)) return true;
      var item = files.pathJoin(dir, itemName);
      if (files.lstat(item).isDirectory()) return search(item);
    }) || false;
  };
...
var updateExistingNpmDirectory = function (packageName, newPackageNpmDir, packageNpmDir, npmDependencies, quiet) {
  ...
  if (!files.stat(packageNpmDir).isDirectory()) throw new Error("Corrupted .npm directory -- should be a directory: " + packageNpmDir);
  if (!files.exists(files.pathJoin(packageNpmDir, 'npm-shrinkwrap.json'))) throw new Error("Corrupted .npm directory -- can't find npm-shrinkwrap.json in " + packageNpmDir);
  ...
  if (files.exists(nodeModulesDir)) {
    ...
    if (oldNodeVersion !== currentNodeCompatibilityVersion()) files.rm_recursive(nodeModulesDir);
  }
  ...
  if (files.exists(nodeModulesDir) && (!files.exists(files.pathJoin(nodeModulesDir, '.package.json')) || !files.exists(files.pathJoin(nodeModulesDir, '.npm-shrinkwrap.json')))) {
  ...
  if (!files.exists(nodeModulesDir)) files.mkdir(nodeModulesDir);
  ...
  var shrinkwrappedDependencies = treeToDependencies(shrinkwrappedDependenciesTree);
  ...
  if (!quiet) logUpdateDependencies(packageName, npmDependencies);
  ...
  _.each(shrinkwrappedDependencies, function (version, name) {
    if (npmDependencies[name] === version) {
      ...
      preservedShrinkwrap.dependencies[name] = shrinkwrappedDependenciesTree.dependencies[name];
    }
  });
  ...
  if (!_.isEmpty(preservedShrinkwrap.dependencies)) {
    ...
    files.writeFile(files.pathJoin(newPackageNpmDir, 'npm-shrinkwrap.json'), JSON.stringify(preservedShrinkwrap, null, /*legible*/2));

    // construct a matching package.json to make `npm install` happy
    constructPackageJson(packageName, newPackageNpmDir, treeToDependencies(preservedShrinkwrap));
...
var completeNpmDirectory = function (packageName, newPackageNpmDir, packageNpmDir, npmDependencies) {
  ...
  files.rename(files.pathJoin(newPackageNpmDir, 'package.json'), files.pathJoin(newPackageNpmDir, 'node_modules', '.package.json'));
...
var runNpmCommand = function (args, cwd) {
  ...
  if (os.platform() === "win32") {
    npmPath = files.convertToOSPath(files.pathJoin(nodeBinDir, "npm.cmd"));
  } else {
  ...
  if (cwd) cwd = files.convertToOSPath(cwd);
  ...
  var env = files.currentEnvWithPathsAdded(nodeBinDir);

  var opts = { cwd: cwd, env: env, maxBuffer: 10 * 1024 * 1024 };

  var future = new Future();
  var child_process = require('child_process');
  child_process.execFile(npmPath, args, opts, function (err, stdout, stderr) {
    if (meteorNpm._printNpmCalls) process.stdout.write(err ? 'failed\n' : 'done\n');
...
var constructPackageJson = function (packageName, newPackageNpmDir, npmDependencies) {
...
var canonicalVersion = function (depObj) {
  if (utils.isUrlWithSha(depObj.from)) return depObj.from;else return depObj.version;
};
...
var installNpmModule = function (name, version, dir) {
  ...
  var installArg = utils.isUrlWithSha(version) ? version : name + "@" + version;
...
var installFromShrinkwrap = function (dir) {
  if (!files.exists(files.pathJoin(dir, "npm-shrinkwrap.json"))) throw new Error("Can't call `npm install` without a npm-shrinkwrap.json file present");
...
var ensureConnected = function () {
  try {
    ...
    buildmessage.error("Can't install npm dependencies. " + "Are you connected to the internet?");
    // Recover by returning false from updateDependencies
    throw new NpmFailure();
  }
};
...
var minimizeShrinkwrap = function (dir) {
  ...
  files.writeFile(files.pathJoin(dir, 'npm-shrinkwrap.json'),
