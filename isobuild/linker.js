...
var _lruCache2 = babelHelpers.interopRequireDefault(_lruCache);

var _utilsUtilsJs = require('../utils/utils.js');

var _jsAnalyzeJs = require('./js-analyze.js');
...
var APP_PRELINK_CACHE = new _lruCache2['default']({
  max: CACHE_SIZE,
  length: function (prelinked) {
    return prelinked.source.length + _utilsUtilsJs.sourceMapLength(prelinked.sourceMap);
  }
});

var packageDot = function (name) {
  if (/^[a-zA-Z][a-zA-Z0-9]*$/.exec(name)) return "Package." + name;else return "Package['" + name + "']";
};
  ...
  maxLineLength: function (ignoreOver) {
    ...
    _.each(self.files, function (file) {
      ...
      _.each(file.source.split('\n'), function (line) {
        if (line.length <= ignoreOver && line.length > m) m = line.length;
      });
      maxInFile.push(m);
    });
  ...
  getPrelinkedFiles: Profile("linker Module#getPrelinkedFiles", function () {
    ...
    if (self.useGlobalNamespace) {
      return _.map(self.files, function (file) {
        ...
        var results = Profile.time("getPrelinkedFiles toStringWithSourceMap (app)", function () {
    ...
    var results = Profile.time('getPrelinkedFiles toStringWithSourceMap (packages)', function () {
      return node.toStringWithSourceMap({
...
var buildSymbolTree = function (symbolMap) {
  ...
  _.each(symbolMap, function (value, symbol) {
    ...
    _.each(parts, function (part) {
      if (!(part in walk)) walk[part] = {};
      walk = walk[part];
    });

    if (value) walk[lastPart] = value;
  });
...
_.extend(File.prototype, {
  ...
  computeAssignedVariables: Profile("linker File#computeAssignedVariables", function () {
    ...
    try {
      return ASSIGNED_GLOBALS_CACHE[self.sourceHash] = _.keys(_jsAnalyzeJs.findAssignedGlobals(self.source));
    } catch (e) {
  ...
  getPrelinkedOutput: Profile("linker File#getPrelinkedOutput", function (options) {
    ...
    var smc = result.map && new sourcemap.SourceMapConsumer(result.map);
    ...
    if (result.code) {
      ...
      smc ? sourcemap.SourceNode.fromStringWithSourceMap(result.code, smc) : result.code);
...
var banner = function (lines, bannerWidth) {
  if (!bannerWidth) bannerWidth = 6 + _.max(lines, function (x) {
    return x.length;
  }).length;
...
// sourceMap.
var fullLink = Profile("linker.fullLink", function (inputFiles, _ref) {
  var useGlobalNamespace = _ref.useGlobalNamespace;
  var combinedServePath = _ref.combinedServePath;
  var name = _ref.name;
  var declaredExports = _ref.declaredExports;
  var imports = _ref.imports;
  var importStubServePath = _ref.importStubServePath;
  var includeSourceMapInstructions = _ref.includeSourceMapInstructions;

  buildmessage.assertInJob();

  var module = new Module({
    name: name, useGlobalNamespace: useGlobalNamespace, combinedServePath: combinedServePath,
    noLineNumbers: false
  });
  ...
  // the static analysis mutates the sources, so this has to be done before
  // concatenation.
  var assignedVariables = undefined;
  ...
  if (failed) return []; // recover by pretending there are no files
  ...
  return _.map(prelinkedFiles, function (file) {
    if (file.sourceMap) {
      if (includeSourceMapInstructions) header = SOURCE_MAP_INSTRUCTIONS_COMMENT + "\n\n" + header;
      ...
      sourceMap.mappings = new Array(headerLines + 1).join(';') + sourceMap.mappings;
      return {
