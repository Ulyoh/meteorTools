...
exports.ignoreFiles = [/~$/, /^\.#/, /^#.*#$/, // emacs swap files
/^\..*\.sw.$/, // vim swap files
/^\.DS_Store\/?$/, /^ehthumbs\.db$/, /^Icon.$/, /^Thumbs\.db$/, /^\.meteor\/$/, /* avoids scanning N^2 files when bundling all packages */
/^\.git\/$/ /* often has too many files to watch */
];

var rejectBadPath = function (p) {
  if (p.match(/\.\./)) throw new Error("bad path: " + p);
};
...
var File = (function () {
  function File(options) {
    ...
    if (options.data && !(options.data instanceof Buffer)) throw new Error('File contents must be provided as a Buffer');
    if (!options.sourcePath && !options.data) throw new Error("Must provide either sourcePath or data");
  ...
  File.prototype.contents = function contents(encoding) {
    if (!this._contents) {
      ...
      } else this._contents = files.readFile(this.sourcePath);
    }
  ...
  File.prototype.setContents = function setContents(b) {
    if (!(b instanceof Buffer)) throw new Error("Must set contents to a Buffer");
    this._contents = b;
    // Un-cache hash.
    this._hash = null;
  };
  ...
  File.prototype.addCacheBuster = function addCacheBuster() {
    if (!this.url) throw new Error("File must have a URL");
    if (this.cacheable) return; // eg, already got setUrlToHash
  ...
  File.prototype.setTargetPathFromRelPath = function setTargetPathFromRelPath(relPath) {
    // XXX hack
    if (relPath.match(/^packages\//) || relPath.match(/^assets\//)) this.targetPath = relPath;else this.targetPath = files.pathJoin('app', relPath);
  ...
  File.prototype.setAssets = function setAssets(assets) {
    if (!_.isEmpty(assets)) this.assets = assets;
  };
...
var Target = (function () {
  ...
  // ... see subclasses for additional options
  {
    ...
    this.providePackageJSONForUnavailableBinaryDeps = providePackageJSONForUnavailableBinaryDeps;
  }
  ...
  Target.prototype.make = function make(_ref2) {
    var _this = this;

    var packages = _ref2.packages;
    var minifyMode = _ref2.minifyMode;
    var addCacheBusters = _ref2.addCacheBusters;
    var minifiers = _ref2.minifiers;
  ...
  Target.prototype._determineLoadOrder = function _determineLoadOrder(_ref3) {
    var _this2 = this;

    var packages = _ref3.packages;
    ...
    buildmessage.enterJob('linking the program', function () {
      ...
      packages.forEach(function (p) {
        ...
        if (p.debugOnly && _this2.buildMode !== 'development') {
          return;
        }
  ...
  Target.prototype._emitResources = function _emitResources(sourceBatches) {
    ...
    sourceBatches.forEach(function (sourceBatch) {
      ...
      resources.forEach(function (resource) {
        if (resource.type === 'asset') return; // already handled
        ...
        if (_.contains(['js', 'css'], resource.type)) {
          ...
          if (resource.type === 'js' && isOs) {
            ...
            if (resource.servePath !== '/packages/global-imports.js') f.setAssets(unibuildAssets);

            if (!isApp && unibuild.nodeModulesPath) {
              var nmd = _this3.nodeModulesDirectories[unibuild.nodeModulesPath];
              if (!nmd) {
                nmd = new NodeModulesDirectory({
                  ...
                  // other!
                  preferredBundlePath: files.pathJoin('npm', colonConverter.convert(unibuild.pkg.name), 'node_modules'),
                  npmDiscards: unibuild.pkg.npmDiscards
                });
                _this3.nodeModulesDirectories[unibuild.nodeModulesPath] = nmd;
              }
              f.nodeModulesDirectory = nmd;

              if (!archinfo.matches(_this3.arch, unibuild.arch)) {
                ...
                // that leads to this.
                if (!_this3.providePackageJSONForUnavailableBinaryDeps) {
                  throw Error("mismatched arch without special feature enabled " + unibuild.pkg.name + " / " + _this3.arch + " / " + unibuild.arch);
                }
                if (!files.exists(files.pathJoin(nmd.sourcePath, '.package.json'))) {
                  buildmessage.error("Can't cross-compile package " + unibuild.pkg.name + ": missing .package.json");
                  return;
                }
                if (!files.exists(files.pathJoin(nmd.sourcePath, '.npm-shrinkwrap.json'))) {
                  buildmessage.error("Can't cross-compile package " + unibuild.pkg.name + ": missing .npm-shrinkwrap.json");
                  return;
                }
        ...
        if (_.contains(['head', 'body'], resource.type)) {
          if (!isWeb) throw new Error('HTML segments can only go to the client');
  ...
  Target.prototype.rewriteSourceMaps = function rewriteSourceMaps() {
    ...
    if (this.js) {
      this.js.forEach(function (js) {
        if (js.sourceMap) js.sourceMap = rewriteSourceMap(js.sourceMap);
      });
    }

    if (this.css) {
      this.css.forEach(function (css) {
        if (css.sourceMap) css.sourceMap = rewriteSourceMap(css.sourceMap);
      });
    }
  };
  ...
  Target.prototype._addCordovaDependency = function _addCordovaDependency(name, version, override) {
    if (!this.cordovaDependencies) return;
...
var ClientTarget = (function (_Target) {
  ...
  function ClientTarget(options) {
    babelHelpers.classCallCheck(this, ClientTarget);

    _Target.call(this, options);
    ...
    if (!archinfo.matches(this.arch, 'web')) throw new Error('ClientTarget targeting something that isn\'t a client?');
  }
  ...
  ClientTarget.prototype.write = function write(builder, _ref4) {
    var _this8 = this;
   ...
   // console.log("bundler.js L1153");
    ...
    var eachResource = (function (f) {
      ...
      ["js", "css", "asset"].forEach(function (type) {
        _this7[type].forEach(function (file) {
          f(file, type);
        });
      });
    }).bind(this);
...
var JsImage = (function () {
  function JsImage() {
    babelHelpers.classCallCheck(this, JsImage);
  ...
  JsImage.prototype.load = function load(bindings) {
    ...
    var getAsset = function (assets, assetPath, encoding, callback) {
      ...
      var fut;
      if (!callback) {
        if (!Fiber.current) throw new Error("The synchronous Assets API can " + "only be called from within a Fiber.");
        fut = new Future();
      ...
      var env = _.extend({
        ...
        Npm: {
          require: function (name) {
            ...
            var nodeModuleDir = files.pathJoin(item.nodeModulesDirectory.sourcePath, name);
            var nodeModuleTopDir = files.pathJoin(item.nodeModulesDirectory.sourcePath, name.split("/")[0]);
  ...
  JsImage.prototype.write = function write(builder, options) {
    var self = this;
    options = options || {};
    ...
    _.each(self.nodeModulesDirectories || [], function (nmd) {
      ...
      if (!options.includeNodeModules || options.includeNodeModules === 'symlink') {
        ...
        // Fail hard.
        throw new Error("Option includeNodeModules wasn't falsy, 'symlink', " + "or 'reference-directly'. It was: " + options.includeNodeModules);
      }
    ...
    var load = [];
    _.each(self.jsToLoad, function (item) {
      if (!item.targetPath) throw new Error("No targetPath?");
      ...
      if (item.nodeModulesDirectory) {
        ...
        var generatedNMD = _.findWhere(nodeModulesDirectories, { sourcePath: item.nodeModulesDirectory.sourcePath });
        if (generatedNMD) {
      ...
      if (item.sourceMap) {
        ...
        if (item.sourceMapRoot) loadItem.sourceMapRoot = item.sourceMapRoot;
      }
    ...
    _.each(nodeModulesDirectories, function (nmd) {
      if (nmd.writePackageJSON) {
        ...
        // elsewhere).
        builder.reserve(nmd.preferredBundlePath, { directory: true });
        // We check that these source files exist in _emitResources when
        // writePackageJSON is initially set.
        builder.write(files.pathJoin(files.pathDirname(nmd.preferredBundlePath), 'package.json'), { file: files.pathJoin(nmd.sourcePath, '.package.json') });
        builder.write(files.pathJoin(files.pathDirname(nmd.preferredBundlePath), 'npm-shrinkwrap.json'), { file: files.pathJoin(nmd.sourcePath, '.npm-shrinkwrap.json') });
  ...
  JsImage.readFromDisk = function readFromDisk(controlFilePath) {
    ...
    if (json.format !== "javascript-image-pre1") throw new Error("Unsupported plugin format: " + JSON.stringify(json.format));
    ...
    _.each(json.load, function (item) {
      ...
      if (item.node_modules) {
        ...
        if (!(node_modules in ret.nodeModulesDirectories)) {
          ret.nodeModulesDirectories[node_modules] = new NodeModulesDirectory({
...
var JsImageTarget = (function (_Target2) {
  babelHelpers.inherits(JsImageTarget, _Target2);

  function JsImageTarget(options) {
    babelHelpers.classCallCheck(this, JsImageTarget);

    _Target2.call(this, options);
...
var ServerTarget = (function (_JsImageTarget) {
  babelHelpers.inherits(ServerTarget, _JsImageTarget);
  ...
  function ServerTarget(options) {
    babelHelpers.classCallCheck(this, ServerTarget);

    for (var _len = arguments.length, args = Array(_len > 1 ? _len - 1 : 0), _key = 1; _key < _len; _key++) {
      args[_key - 1] = arguments[_key];
    }

    _JsImageTarget.call.apply(_JsImageTarget, [this, options].concat(args));

    this.clientTargets = options.clientTargets;
    this.releaseName = options.releaseName;

    if (!archinfo.matches(this.arch, "os")) throw new Error("ServerTarget targeting something that isn't a server?");
  }
  ...
  ServerTarget.prototype.write = function write(builder, options) {
    var self = this;
    var nodePath = [];
    ...
    if (options.includeNodeModules === 'symlink') {
      ...
      nodePath.push(files.pathJoin(files.getDevBundle(), 'server-lib', 'node_modules'));
    } else if (options.includeNodeModules) {
      // This is some option we didn't expect - someone has added another case
      // to the includeNodeModules option but didn't update this if block. Fail
      // hard.
      throw new Error("Option includeNodeModules wasn't falsy, 'symlink', " + "or 'reference-directly'.");
    }
    ...
    // Server bootstrap
    _.each(["boot.js", "boot-utils.js", "shell-server.js", "mini-files.js"], function (filename) {
      builder.write(filename, {
        file: files.pathJoin(files.pathDirname(files.convertToStandardPath(__dirname)), 'static-assets', 'server', filename)
      });
    });
...
var writeFile = Profile("bundler..writeFile", function (file, builder) {
  if (!file.targetPath) throw new Error("No targetPath?");
  var contents = file.contents();
  if (!(contents instanceof Buffer)) throw new Error("contents not a Buffer?");
...
var writeTargetToPath = Profile("bundler..writeTargetToPath", function (name, target, outputPath, _ref5) {
  var includeNodeModules = _ref5.includeNodeModules;
  var getRelativeTargetPath = _ref5.getRelativeTargetPath;
  var previousBuilder = _ref5.previousBuilder;
  var minifyMode = _ref5.minifyMode;
  ...
  var targetBuild = target.write(builder, { includeNodeModules: includeNodeModules, getRelativeTargetPath: getRelativeTargetPath, minifyMode: minifyMode });
...
var writeSiteArchive = Profile("bundler..writeSiteArchive", function (targets, outputPath, _ref6) {
  var includeNodeModules = _ref6.includeNodeModules;
  var builtBy = _ref6.builtBy;
  var releaseName = _ref6.releaseName;
  var getRelativeTargetPath = _ref6.getRelativeTargetPath;
  var previousBuilders = _ref6.previousBuilders;
 ...
 * you are testing!
 */
exports.bundle = function (_ref7) {
  var projectContext = _ref7.projectContext;
  var outputPath = _ref7.outputPath;
  var includeNodeModules = _ref7.includeNodeModules;
  var buildOptions = _ref7.buildOptions;
  var previousBuilders = _ref7.previousBuilders;
  var hasCachedBundle = _ref7.hasCachedBundle;
  var providePackageJSONForUnavailableBinaryDeps = _ref7.providePackageJSONForUnavailableBinaryDeps;

  buildOptions = buildOptions || {};

  var serverArch = buildOptions.serverArch || archinfo.host();
  var webArchs = buildOptions.webArchs || projectContext.platformList.getWebArchs();
  ...
  if (!release.usingRightReleaseForApp(projectContext)) throw new Error("running wrong release for app?");
  ...
  var messages = buildmessage.capture({
    ...
    var makeServerTarget = Profile("bundler.bundle..makeServerTarget", function (app, clientTargets) {
      ...
      if (clientTargets) targetOptions.clientTargets = clientTargets;
    ...
    var mergeAppWatchSets = function () {
    ...
    if (outputPath !== null) {
      if (hasCachedBundle) {
        ...
        _.each(targets, function (target, name) {
          var previousBuilder = previousBuilders && previousBuilders[name];
          var targetBuild = writeTargetToPath(name, target, outputPath, _.extend({}, writeOptions, { previousBuilder: previousBuilder }));
          nodePath = nodePath.concat(targetBuild.nodePath);
          clientWatchSet.merge(target.getWatchSet());
          builders[name] = targetBuild.builder;
        });
      } else {
        starResult = writeSiteArchive(targets, outputPath, _.extend({}, writeOptions, { previousBuilders: previousBuilders }));
  ...
  if (success && messages.hasMessages()) success = false; // there were errors
...
exports.buildJsImage = Profile("bundler.buildJsImage", function (options) {
  buildmessage.assertInCapture();
  if (options.npmDependencies && !options.npmDir) throw new Error("Must indicate .npm directory to use");
  if (!options.name) throw new Error("Must provide a name");
