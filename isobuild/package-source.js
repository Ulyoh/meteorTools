...
_.extend(PackageSource.prototype, {
  ...
  initFromOptions: function (name, options) {
    ...
    if (options.sources && !_.isEmpty(options.sources) && (!options.sourceRoot || !options.serveRoot)) throw new Error("When source files are given, sourceRoot and " + "serveRoot must be specified");
    ...
    if (!self._checkCrossUnibuildVersionConstraints()) throw new Error("only one unibuild, so how can consistency check fail?");
  },
  ...
  initFromPackageDir: function (dir, options) {
    ...
    if (!files.exists(self.sourceRoot)) throw new Error("putative package directory " + dir + " doesn't exist?");
            ...
            if (typeof value !== "string") {
              ...
              // Recover by pretending that version was not set.
                ...
                try {
                  ...
                  if (!e.versionParserError) throw e;
    ...
    if (self.debugOnly && !_.isEmpty(self.pluginInfo)) {
      buildmessage.error("can't register build plugins in debugOnly packages");
      // recover by ignoring
    }
    ...
    // Meteor.)
    var preLinkerFiles = ['npm-shrinkwrap.json', 'README', '.gitignore', 'node_modules'];
    ...
    _.each(compiler.ALL_ARCHES, function (arch) {
      ...
      if (self.name !== "meteor" && !process.env.NO_METEOR_PACKAGE) {
        ...
        if (!alreadyDependsOnMeteor) api.uses[arch].unshift({ 'package': "meteor" });
      }
  ...
  initFromAppDir: Profile("initFromAppDir", function (projectContext, ignoreFiles) {
    ...
    _.each(compiler.ALL_ARCHES, function (arch) {
      ...
      sourceArch.getFiles = function (sourceProcessorSet, watchSet) {
        var _sourceReadOptions$exclude;
        ...
        (_sourceReadOptions$exclude = sourceReadOptions.exclude).push.apply(_sourceReadOptions$exclude, ignoreFiles);

        // Wrapper around watch.readAndWatchDirectory which takes in and returns
        // sourceRoot-relative directories.
        var readAndWatchDirectory = function (relDir, _ref) {
          var include = _ref.include;
          var exclude = _ref.exclude;
          var names = _ref.names;

          var absPath = files.pathJoin(self.sourceRoot, relDir);
          var contents = watch.readAndWatchDirectory(watchSet, { absPath: absPath, include: include, exclude: exclude, names: names });
        ...
        var otherUnibuildRegExp = arch === "os" ? /^client\/$/ : /^server\/$/;
        ...
        var sourceDirectories = readAndWatchDirectory('', {
          ...
          // project.
          /^node_modules\/$/, /^public\/$/, /^private\/$/, /^cordova-build-override\/$/, otherUnibuildRegExp].concat(sourceReadOptions.exclude)
        });
        ...
        while (!_.isEmpty(sourceDirectories)) {
          ...
          if (checkForInfiniteRecursion(dir)) return []; // pretend we found no files

          // Find source files in this directory.
          (_sources = sources).push.apply(_sources, readAndWatchDirectory(dir, sourceReadOptions));

          // Find sub-sourceDirectories. Note that we DON'T need to ignore the
          // directory names that are only special at the top level.
          sourceDirectories.push.apply(sourceDirectories, readAndWatchDirectory(dir, {
            include: [/\/$/],
            exclude: [/^tests\/$/, otherUnibuildRegExp].concat(sourceReadOptions.exclude)
          }));
        }
        ...
        sources = _.map(sources, function (relPath) {
          ...
          if (archinfo.matches(arch, "web") && relPath.match(/\.js$/)) {
            var clientCompatSubstr = files.pathSep + 'client' + files.pathSep + 'compatibility' + files.pathSep;

            if ((files.pathSep + relPath).indexOf(clientCompatSubstr) !== -1) sourceObj.fileOptions = { bare: true };
          }
          return sourceObj;
        });
        ...
        if (!_.isEmpty(assetDirs)) {
          if (!_.isEqual(assetDirs, [assetDir])) throw new Error("Surprising assetDirs: " + JSON.stringify(assetDirs));
  ...
  getDependencyMetadata: function (options) {
    ...
    if (!ret) {
      if (options.logError) return null;else throw new Error("inconsistent dependency constraint across unibuilds?");
    }
    return ret;
  },
  ...
  getPackagesToLoadFirst: function (packageMap) {
    ...
    var processUse = function (use) {
      ...
      if (use.weak || use.unordered) return;
      ...
      if (!packageInfo) throw Error("Depending on unknown package " + use['package']);
  ...
  _computeDependencyMetadata: function (options) {
    ...
    _.each(self.architectures, function (arch) {
      ...
      var processUse = function (implied, use) {
        ...
        if (use.weak && options.skipWeak || use.unordered && options.skipUnordered) return;
