...
exports.getActivePluginPackages = getActivePluginPackages;
exports.isIsobuildFeaturePackage = isIsobuildFeaturePackage;

var _buildPluginJs = require('./build-plugin.js');
...
compiler.compile = Profile(function (packageSource, options) {
  ...
  _.each(packageSource.pluginInfo, function (info) {
    buildmessage.enterJob({
      ...
      var buildResult = bundler.buildJsImage({
        ...
        npmDir: files.pathResolve(files.pathJoin(packageSource.sourceRoot, '.npm', 'plugin', info.name))
      });
      ...
      if (!_.has(plugins, info.name)) plugins[info.name] = {};
  ...
  if (packageSource.npmCacheDirectory) {
    if (meteorNpm.updateDependencies(packageSource.name, packageSource.npmCacheDirectory, packageSource.npmDependencies)) {
      ...
      if (!process.env.METEOR_FORCE_PORTABLE && !meteorNpm.dependenciesArePortable(packageSource.npmCacheDirectory)) {
        isPortable = false;
      }
    }
  }
  ...
  _.each(packageSource.architectures, function (architecture) {
    if (architecture.arch === 'web.cordova' && !includeCordovaUnibuild) return;
...
function getLinterSourceProcessorSet(_ref) {
  ...
  var sourceProcessorSet = new _buildPluginJs.SourceProcessorSet(isopack.displayName, { allowConflicts: true });
...
var lintUnibuild = function (_ref2) {
  ...
  var unibuild = _.findWhere(isopack.unibuilds, { arch: sourceArch.arch });
  if (!unibuild) {
...
var compileUnibuild = function (options) {
  ...
  // as a static asset).
  var sourceProcessorSet = undefined,
      linterSourceProcessorSet = undefined;
  buildmessage.enterJob("determining active plugins", function () {
    sourceProcessorSet = new _buildPluginJs.SourceProcessorSet(isopk.displayName(), { hardcodeJs: true });
  ...
  var _ref3 = sourceProcessorSet ? inputSourceArch.getFiles(sourceProcessorSet, watchSet) : {};

  var _ref3$sources = _ref3.sources;
  var sources = _ref3$sources === undefined ? [] : _ref3$sources;
  var _ref3$assets = _ref3.assets;
  var assets = _ref3$assets === undefined ? [] : _ref3$assets;
  ...
  var nodeModulesPathOrUndefined = nodeModulesPath;
...
function runLinters(_ref4) {
  var inputSourceArch = _ref4.inputSourceArch;
  var isopackCache = _ref4.isopackCache;
  ...
  compiler.eachUsedUnibuild({
    ...
    if (unibuild.pkg.name === inputSourceArch.pkg.name) return;
  ...
  _.values(sources).forEach(function (sourceItem) {
    ...
    if (classification.type === 'wrong-arch' || classification.type === 'unmatched') return;
    ...
    var _watch$readAndWatchFileWithHash = watch.readAndWatchFileWithHash(watchSet, files.pathResolve(inputSourceArch.pkg.sourceRoot, relPath));

    var hash = _watch$readAndWatchFileWithHash.hash;
    var contents = _watch$readAndWatchFileWithHash.contents;

    var wrappedSource = {
      relPath: relPath, contents: contents, hash: hash,
  ...
  _.each(sourceItemsForLinter, function (_ref5) {
    var sourceProcessor = _ref5.sourceProcessor;
    ...
    function archToString(arch) {
      if (arch.match(/web\.cordova/)) return "Cordova";
      if (arch.match(/web\..*/)) return "Client";
      if (arch.match(/os.*/)) return "Server";
...
function getActivePluginPackages(isopk, _ref6) {
  var uses = _ref6.uses;
  var isopackCache = _ref6.isopackCache;
  var pluginProviderPackageNames = _ref6.pluginProviderPackageNames;
  var pluginProviderWatchSet = _ref6.pluginProviderWatchSet;
  ...
  if (pluginProviderPackageNames) pluginProviderPackageNames[isopk.name] = true;
  ...
  compiler.eachUsedUnibuild({
    ...
    if (unibuild.pkg.name === isopk.name) return;
    ...
    if (_.isEmpty(unibuild.pkg.plugins)) return;
...
compiler.eachUsedUnibuild = function (options, callback) {
  ...
  _.each(dependencies, function (use) {
    if (options.skipUnordered && use.unordered) return;
    if (use.weak && !_.has(acceptableWeakPackages, use['package'])) return;
  ...
  while (!_.isEmpty(usesToProcess)) {
    ...
    if (isIsobuildFeaturePackage(use['package'])) continue;
    ...
    if (_.has(processedUnibuildId, unibuild.id)) continue;
...
exports.KNOWN_ISOBUILD_FEATURE_PACKAGES = KNOWN_ISOBUILD_FEATURE_PACKAGES;
