  ...
  var interval = setInterval(function () {
    if (ddpConnection.status().status === "connected") {
      ...
      ddpConnection.subscribe("VelocityTestReports", {
        onError: function () {
          Console.error("failed to subscribe to VelocityTestReports " + "subscription");
      ...
      ddpConnection.subscribe("VelocityAggregateReports", {
        onError: function () {
          Console.error("failed to subscribe to " + "VelocityAggregateReports subscription");
        }, onReady: function () {
          this.connection.registerStore("velocityAggregateReports", {
            update: function (msg) {
              if (msg.msg === "added" || msg.msg === "changed") {
                ...
                if (report.name === "aggregateComplete" && report.result === "completed") {
                  setTimeout(function () {
      ...
      function visitWithPhantom(url) {
        ...
        var browserProcess = child_process.execFile('/bin/bash', ['-c', "exec " + phantomjs.path + " /dev/stdin <<'END'\n" + phantomScript + "END\n"]);
        browserProcesses.push(browserProcess);
      }

      ddpConnection.subscribe("VelocityMirrors", {
        onError: function (err) {
          Console.error("failed to subscribe to VelocityMirrors " + "subscription", err);
        }, onReady: function () {
          this.connection.registerStore("velocityMirrors", {
            update: function (msg) {
              if (isMirrorUrlMessage(msg)) {
                mirrorUrls[msg.id] = generateMirrorUrl(msg.fields.rootUrl, msg.fields.rootUrlPath);
              }
              if (isMirrorReadyMessage(msg)) {
                ...
                if (mirrorUrl) {
                  ...
                  Console.error("Could not find URL of mirror " + "with the MongoDB ID " + msg.id);
                }
              }
            }
          });
...
//# sourceMappingURL=run-velocity.js.map