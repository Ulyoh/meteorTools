...
_.extend(RunLog.prototype, {
  ...
  logAppOutput: function (line, isStderr) {
    ...
    self._clearSpecial();
    if (self.rawLogs) Console[isStderr ? "rawError" : "rawInfo"](line + "\n");else Console.rawInfo(Log.format(obj, { color: true }) + "\n");
  ...
  logRestart: function () {
    ...
    if (self.consecutiveRestartMessages > 1) message += " (x" + self.consecutiveRestartMessages + ")";
  ...
  logClientRestart: function () {
    ...
    if (self.consecutiveClientRestartMessages > 1) message += " (x" + self.consecutiveClientRestartMessages + ")";
    // no newline, so that we can overwrite it if we get another
    // restart message right after this one
    process.stdout.write(message);
