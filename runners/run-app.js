...
var _cordovaBuilderJs = require('../cordova/builder.js');
...
_.extend(AppProcess.prototype, {
  ...
  start: function () {
    ...
    require('../tool-env/cleanup.js').onExit(function () {
      require('../static-assets/server/shell-server.js').disable(self.projectContext.getMeteorShellDirectory());
    });
  },

  _maybeCallOnExit: function (code, signal) {
    var self = this;
    if (self.madeExitCallback) return;
...
_.extend(AppRunner.prototype, {
  ...
  start: function () {
    ...
    if (self.fiber) throw new Error("already started?");
  ...
  stop: function () {
    ...
    if (!self.fiber) return; // nothing to do

    if (self.exitFuture) throw new Error("another fiber already stopping?");
  ...
  _runOnce: function (options) {
    ...
    var bundleApp = function () {
      ...
      var bundleResult = Profile.run("Rebuild App", function () {
        ...
        builders = bundleResult.builders;
    ...
    if (bundleResultOrRunResult.runResult) return bundleResultOrRunResult.runResult;
    ...
    var settingsMessages = buildmessage.capture({
      ...
      if (self.settingsFile) settings = files.getSettings(self.settingsFile, settingsWatchSet);
    });
    ...
    if (self.runFuture) throw new Error("already have future?");
    ...
    var appProcess = new AppProcess({
      ...
      onListen: function () {
        ...
        if (self.startFuture) self.startFuture['return']();
      },
    ...
    try {
      while (ret.outcome === 'changed-refreshable') {
        if (!canRefreshClient) throw Error("Can't refresh client?");
  ...
  _fiber: function () {
    ...
    while (true) {
      ...
      var runResult = self._runOnce({
        onListen: function () {
          if (!self.noRestartBanner && !firstRun) runLog.logRestart();
        },
      ...
      if (runResult.outcome !== "terminated") crashCount = 0;

      var wantExit = self.onRunEnd ? !self.onRunEnd(runResult) : false;
      if (wantExit || self.exitFuture || runResult.outcome === "stopped") break;
      ...
      } else if (runResult.outcome === "changed") continue;else if (runResult.outcome === "terminated") {
        if (runResult.signal) {
      ...
      if (self.watchForChanges) {
        ...
        if (!runResult.watchSet) throw Error("watching for changes with no watchSet?");
    ...
    // Giving up for good.
    if (self.exitFuture) self.exitFuture['return']();
    if (self.startFuture) self.startFuture['return']();
