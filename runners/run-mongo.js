...
if (process.platform === 'win32') {
  ...
  findMongoPids = function (app_dir, port) {
    ...
    child_process.exec('tasklist /fi "IMAGENAME eq mongod.exe"', function (error, stdout, stderr) {
      if (error) {
        ...
        fut['throw'](new Error("Couldn't run tasklist.exe: " + additionalInfo));
        return;
      } else {
  ...
  findMongoPids = function (appDir, port) {
    ...
    { maxBuffer: 1024 * 1024 * 10 }, function (error, stdout, stderr) {
      if (error) {
        fut['throw'](new Error("Couldn't run ps ax: " + JSON.stringify(error) + "; " + error.message));
        return;
      }
      ...
      _.each(stdout.split('\n'), function (line) {
        ...
        if (m && m.length === 4) {
          ...
          if ((!port || port === foundPort) && (!appDir || appDir === foundPath)) {
            ret.push({
              pid: foundPid,
              port: foundPort,
              appDir: foundPath
...
var findMongoAndKillItDead = function (port, dbPath) {
  ...
  _.each(pids, function (processInfo) {
    ...
    for (var attempts = 1; attempts <= 40; attempts++) {
      var signal = 0;
      if (attempts === 1) signal = 'SIGINT';else if (attempts === 20 || attempts === 30) signal = 'SIGKILL';
...
var launchMongo = function (options) {
  ...
  if (process.env.METEOR_TEST_FAKE_MONGOD_CONTROL_PORT) {
    if (options.multiple) throw Error("Can't specify multiple with fake mongod");
  ...
  var launchOneMongoAndWaitForReadyForInitiate = function (dbPath, port, portFile) {
    ...
    if (options.multiple) {
      ...
      if (!matchingPortFileExists) {
        ...
        if (portFileExists) files.unlink(portFile);
    ...
    var maybeReadyToTalk = function () {
      if (readyToTalkFuture.isResolved()) return;
  ...
  var initiateReplSetAndWaitForReady = function () {
    try {
      ...
      var initiateResult = yieldingMethod(db.admin(), 'command', { replSetInitiate: configuration });
      if (stopped) return;
      // why this isn't in the error is unclear.
      if (initiateResult && initiateResult.documents && initiateResult.documents[0] && initiateResult.documents[0].errmsg) {
      ...
      while (!stopped) {
        ...
        if (!(status && status.documents && status.documents[0])) throw status;
        status = status.documents[0];
        if (!status.ok) {
          if (status.startupStatus === 6) {
            // "SOON"
            utils.sleepMs(20);
            continue;
          }
      ...
      if (!stopped || e instanceof StoppedDuringLaunch) throw e;
    }
  };
  ...
  try {
    ...
    if (!(e instanceof StoppedDuringLaunch)) throw e;
  }
...
_.extend(MRp, {
  ...
  start: function () {
    ...
    if (self.handle) throw new Error("already running?");
  ...
  _exited: function (code, signal, stderr, detectedErrors) {
    ...
    self.errorCount++;
    if (self.errorTimer) clearTimeout(self.errorTimer);
