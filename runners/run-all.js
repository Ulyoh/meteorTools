...
var Runner = (function () {
  function Runner(_ref) {
    ...
    var optionsForAppRunner = babelHelpers.objectWithoutProperties(_ref, ['appHost', 'appPort', 'banner', 'disableOplog', 'cordovaRunner', 'mongoUrl', 'onFailure', 'oplogUrl', 'projectContext', 'proxyHost', 'proxyPort', 'quiet', 'rootUrl', 'selenium', 'seleniumBrowser']);
    ...
    self.appRunner = new AppRunner(babelHelpers._extends({}, optionsForAppRunner, {
  ...
  Runner.prototype.start = function start() {
    ...
    if (!self.stopped) {
      ...
      if (!self.quiet && !self.stopped) runLog.log("Started your app.", { arrow: true });
    }
    ...
    if (self.selenium && !self.stopped) {
      ...
      if (!self.quiet && !self.stopped) runLog.log("Started Selenium.", { arrow: true });
    }
  ...
  Runner.prototype.regenerateAppPort = function regenerateAppPort() {
    ...
    if (self.proxy) self.proxy.proxyToPort = self.appPort;
    if (self.appRunner) self.appRunner.port = self.appPort;
  };
