...
DEp.connectToChildProcess = function connectToChildProcess(child) {
  ...
  child.stderr.on("data", function onData(buffer) {
    var match = /debugger listening on port (\d+)/.exec(buffer.toString("utf8"));
    if (match) {
...
function banner(debugPort) {
  ...
  return ["", chalk.green(["Your application is now paused and ready for debugging!", "", "To debug the server process using a graphical debugging interface, ", "visit this URL in your web browser:"].join(EOL)), chalk.cyan(proc.url), EOL].join(EOL);
}
