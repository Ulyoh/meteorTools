...
var deployRpc = function (options) {
  ...
  if (options.headers.cookie) throw new Error("sorry, can't combine cookie headers yet");

  // XXX: Reintroduce progress for upload
  try {
    var result = httpHelpers.request(_.extend(options, {
      url: config.getDeployUrl() + '/' + options.operation + (options.site ? '/' + options.site : ''),
...
var authedRpc = function (options) {
  ...
  if (infoResult.errorMessage) return infoResult;
...
var bundleAndDeploy = function (options) {
  if (options.recordPackageUsage === undefined) options.recordPackageUsage = true;
  ...
  var messages = buildmessage.capture({
    ...
    if (options.settingsFile) settings = files.getSettings(options.settingsFile);
  });
  ...
  if (!messages.hasMessages()) {
    ...
    if (bundleResult.errors) messages = bundleResult.errors;
  }
...
var changeAuthorized = function (site, action, username) {
  ...
  Console.info(site + ": " + (action === "add" ? "added " : "removed ") + username);
  return 0;
};
...
var claim = function (site) {
  ...
  if (infoResult.payload && infoResult.payload.protection === "account") {
    if (infoResult.payload.authorized) Console.error("That site already belongs to you.\n");else Console.error("Sorry, that site belongs to someone else.\n");
    return 1;
  }
