...
var loggedInAccountsConnection = function (token) {
  ...
  var fut = new Future();
  connection.apply('login', [{ resume: token }], { wait: true }, function (err, result) {
  ...
  return function () {
    ...
    if (options.timeout !== undefined) {
      var timer = setTimeout(fiberHelpers.bindEnvironment(function () {
        if (!fut.isResolved()) fut['throw'](new Error('Method call timed out'));
      }), options.timeout);
    }
    try {
      var result = fut.wait();
      ...
      if (!options.connection) conn.close();
    }
...
var writeSessionData = function (data) {
  ...
  var tries = 0;
  while (true) {
    if (tries++ > 10) throw new Error("can't find a unique name for temporary file?");
...
var getSession = function (sessionData, domain) {
  if (typeof sessionData.sessions !== "object") sessionData.sessions = {};
  if (typeof sessionData.sessions[domain] !== "object") sessionData.sessions[domain] = {};
...
var ensureSessionType = function (session, type) {
  if (!_.has(session, 'type')) session.type = type;else if (session.type !== type) {
...
var logOutSession = function (session) {
  ...
  if (_.has(session, 'token')) {
    if (!(session.pendingRevoke instanceof Array)) session.pendingRevoke = [];
    ...
    // server side too.
    if (typeof session.tokenId === "string") session.pendingRevoke.push(session.tokenId);
...
var removePendingRevoke = function (domain, tokenIds) {
  ...
  if (!session.pendingRevoke.length) delete session.pendingRevoke;
  writeSessionData(data);
};
...
var tryRevokeOldTokens = function (options) {
  ...
  _.each(readSessionData().sessions || {}, function (session, domain) {
    if (session.pendingRevoke && session.pendingRevoke.length) domainsWithRevokedTokens.push(domain);
  });
...
var doInteractivePasswordLogin = function (options) {
  ...
  if (_.has(options, 'username')) loginData.username = options.username;else if (_.has(options, 'email')) loginData.email = options.email;else throw new Error("Need username or email");
...
exports.logoutCommand = function (options) {
  ...
  if (wasLoggedIn) Console.error("Logged out.");else
...
exports.pollForRegistrationCompletion = function (options) {
  if (alreadyPolledForRegistration) return;
...
exports.registerOrLogIn = withAccountsConnection(function (connection) {
  ...
  if (!result.alreadyExisted) {
    ...
    try {
      ...
      stopSpinner();
      if (e.errorType !== "Meteor.Error") throw e;
...
exports.maybePrintRegistrationLink = function (options) {
  ...
  if (session.userId && !session.username && session.registrationUrl) {
    if (options.leadingNewline) Console.error();
