...
var ServiceConnection = function (endpointUrl, options) {
  ...
  options = _.extend({}, options, {
    ...
    onConnected: function () {
      self.connected = true;
      if (!self.currentFuture) throw Error("nobody waiting for connection?");
      if (self.currentFuture !== connectFuture) throw Error("waiting for something that isn't connection?");
  ...
  self.connection._stream.on('disconnect', function (error) {
    ...
    if (error && error.errorType === "DDP.ForcedReconnectError") {
      ...
      if (connectFuture.isResolved()) throw Error("disconnect before connect?");
    ...
    if (self.currentFuture) {
      ...
      self.currentFuture = null;
      fut["throw"](error || new Package['ddp-client'].DDP.ConnectionError("DDP disconnected while connection in progress"));
    } else if (error) {
...
_.extend(ServiceConnection.prototype, {
  ...
  apply: function () {
    ...
    if (self.currentFuture) throw Error("Can't wait on two things at once!");
