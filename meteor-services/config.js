...
var getUniverse = function () {
  if (!universe) {
    ...
    if (files.inCheckout()) {
      ...
      if (files.exists(p)) universe = files.readFile(p, 'utf8').trim();
    }
  }
...
var localhostOffset = function (portOffset) {
  ...
  if (!match) throw new Error("not a local universe?");
  return "localhost:" + (parseInt(match[2] || "80") + portOffset);
};

var getAuthServiceHost = function () {
  if (!isLocalUniverse()) return universe;else
...
var addScheme = function (host) {
  if (host.match(/^localhost(:\d+)?$/)) return "http://" + host;else return "https://" + host;
};
...
_.extend(exports, {
  ...
  getBuildFarmUrl: function () {
    if (process.env.METEOR_BUILD_FARM_URL) return process.env.METEOR_BUILD_FARM_URL;
  ...
  getPackageServerUrl: function () {
    if (process.env.METEOR_PACKAGE_SERVER_URL) return process.env.METEOR_PACKAGE_SERVER_URL;
  ...
  getDeployUrl: function () {
    ...
    if (process.env.DEPLOY_HOSTNAME) {
      ...
      if (host.match(/^http/)) return host; // allow it to contain a URL scheme
    } else {
        // Otherwise, base it on the universe.
        if (isLocalUniverse()) throw new Error("local development of deploy server not supported");else host = getUniverse().replace(/^www\./, 'deploy.');
      }
  ...
  getUpdateManifestUrl: function () {
    if (isLocalUniverse()) u = "www.meteor.com"; // localhost can't run the manifest server
  ...
  getDiscoveryPort: function () {
    if (process.env.DISCOVERY_PORT) return parseInt(process.env.DISCOVERY_PORT);else return 443;
  },
  ...
  printUniverseBanner: function () {
    if (!config.isProduction()) process.stderr.write('[Universe: ' + config.getUniverse() + ']\n');
  }
});
