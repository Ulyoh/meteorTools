...
exports.formatUrl = function (url) {
  var string = url.protocol + url.host;
  if (url.port) {
    string += ':' + url.port;
  }
  return string;
...
exports.parsePackageConstraint = function (constraintString, options) {
  try {
    ...
    if (!(e.versionParserError && options && options.useBuildmessage)) throw e;
...
exports.ensureOnlyExactVersions = function (dependencies) {
  _.each(dependencies, function (version, name) {
    ...
    if (!exports.isExactVersion(version)) {
      throw new Error("Must declare exact version of dependency: " + name + '@' + version);
    }
  });
};
exports.isExactVersion = function (version) {
  return semver.valid(version) || exports.isUrlWithSha(version) || exports.isUrlWithFileScheme(version);
};
