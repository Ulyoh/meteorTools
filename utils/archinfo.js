...
var host = function () {
  if (!_host) {
    var run = function () {
      ...
      if (!result) throw new Error("can't get arch with " + args.join(" ") + "?");
    ...
    if (platform === "darwin") {
      ...
      // return other values.
      if (run('uname', '-p') !== "i386" || run('sysctl', '-n', 'hw.cpu64bit_capable') !== "1") throw new Error("Only 64-bit Intel processors are supported on OS X");
      _host = "os.osx.x86_64";
    } else if (platform === "linux") {
      var machine = run('uname', '-m');
      if (_.contains(["i386", "i686", "x86"], machine)) _host = "os.linux.x86_32";else if (_.contains(["x86_64", "amd64", "ia64"], machine)) _host = "os.linux.x86_64";else throw new Error("Unsupported architecture: " + machine);
...
var mostSpecificMatch = function (host, programs) {
  ...
  _.each(programs, function (p) {
    if (seen[p]) throw new Error("Duplicate architecture: " + p);
    seen[p] = true;
    if (archinfo.matches(host, p) && (!best || p.length > best.length)) best = p;
  });
...
var leastSpecificDescription = function (programs) {
  ...
  _.each(programs, function (p) {
    if (!archinfo.matches(longest, p)) throw new Error("Incompatible architectures: '" + p + "' and '" + longest + "'");
  });
...
var withoutSpecificOs = function (arch) {
  if (arch.substr(0, 3) === 'os.') return 'os';
  return arch;
};
