...
var convertBySchema = function (val, schema) {
  if (schema === true) return convert(val);else if (schema === false) return val;
  ...
  if (!_.isObject(schema)) throw new Error("Unexpected type of schema: " + typeof schema);

  var ret = _.clone(val);
  _.each(schema, function (subschema, key) {
    if (_.has(ret, key)) ret[key] = convertBySchema(val[key], subschema);
  });
