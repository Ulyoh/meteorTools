...
var getUserAgent = function () {
  ...
  if (release.current) version = release.current.isCheckout() ? 'checkout' : release.current.name;else
...
_.extend(exports, {
  ...
  request: function (urlOrOptions, callback) {
    var options;
    if (!_.isObject(urlOrOptions)) options = { url: urlOrOptions };else options = _.clone(urlOrOptions);
    ...
    if (useSessionHeader || useAuthHeader) {
      ...
      if (sessionHeader) options.headers['X-Meteor-Session'] = sessionHeader;
      if (callback) throw new Error("session header can't be used with callback");
    }
    if (useAuthHeader) {
      var authHeader = auth.getSessionToken(config.getAccountsDomain());
      if (authHeader) options.headers['X-Meteor-Auth'] = authHeader;
    }
    ...
    if (!callback) {
      ...
      callback = function (err, response, body) {
        ...
        _.each(response.headers["set-cookie"] || [], function (h) {
          ...
          if (match) setCookie[match[1]] = match[2];
        });
