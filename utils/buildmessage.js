    ...
    _.each(self.messages, function (message) {
      ...
      if (stack.length > 1) {
        _.each(stack, function (frame) {
          ...
          if (!frame.func && !where) return; // that's a pretty lame stack frame

          line += "  at ";
          if (frame.func) line += frame.func + " (" + where + ")\n";else line += where + "\n";
        });
        line += "\n";
      }
...
var capture = function (options, f) {
  ...
  var title;
  if (typeof options === "object" && options.title) title = options.title;
...
var jobHasMessages = function () {
  var search = function (job) {
    if (job.hasMessages()) return true;
...
var error = function (message, options) {
  ...
  if (options.downcase) message = message.slice(0, 1).toLowerCase() + message.slice(1);

  if (!currentJob.get()) throw new Error("Error: " + message);

  if (options.secondary && jobHasMessages()) return; // skip it
...
var assertInJob = function () {
  if (!currentJob.get()) throw new Error("Expected to be in a buildmessage job");
};

var assertInCapture = function () {
  if (!currentMessageSet.get()) throw new Error("Expected to be in a buildmessage capture");
};

var mergeMessagesIntoCurrentJob = function (innerMessages) {
  var outerMessages = currentMessageSet.get();
  if (!outerMessages) throw new Error("Expected to be in a buildmessage capture");
  var outerJob = currentJob.get();
  if (!outerJob) throw new Error("Expected to be in a buildmessage job");
