...
exports.waitForOne = function () {
  var fiber = Fiber.current;
  if (!fiber) throw Error("Can't waitForOne without a fiber");

  for (var _len2 = arguments.length, futures = Array(_len2), _key2 = 0; _key2 < _len2; _key2++) {
    futures[_key2] = arguments[_key2];
  }

  if (futures.length === 0) throw Error("Must wait for at least one future");
...
_.extend(exports.EnvironmentVariable.prototype, {
  get: function () {
    ...
    if (!Fiber.current._meteorDynamics) return self.defaultValue;
    if (!_.has(Fiber.current._meteorDynamics, self.slot)) return self.defaultValue;
  ...
  withValue: function (value, func) {
    ...
    if (!Fiber.current._meteorDynamics) Fiber.current._meteorDynamics = {};
