...
RequireInvocation.prototype.isOurCode = function () {
  ...
  if (!self.filename) return self.name === 'TOP';

  if (!self.name.match(/\//)) return false; // we always require our stuff via a path

  var ourSource = path.resolve(__dirname);
  var required = path.resolve(path.dirname(self.filename), self.name);
  if (ourSource.length > required.length) return false;
...
RequireInvocation.prototype.why = function () {
  ...
  if (!walk) return "???";
  if (last) return path.basename(walk.name) + ":" + path.basename(last.name);
...
exports.printReport = function () {
  ...
  var computeTimes = function (inv) {
    ...
    if (inv.totalTime !== null) inv.selfTime = inv.totalTime - childTime;
  };
  ...
  var summarize = function (inv, depth) {
    ...
    if (!(inv.name in summary)) summary[inv.name] = { name: inv.name, time: 0, ours: inv.isOurCode(),
      via: {} };
  ...
  _.each(times, function (item) {
    ...
    if (!item.ours) line += " [via " + _.keys(item.via).join(", ") + "]";
    console.log(line);
    if (item.ours) ourTotal += item.time;else otherTotal += item.time;
  });

  var grandTotal = currentInvocation.totalTime;
  if (grandTotal - ourTotal - otherTotal > 1 / 1000) throw new Error("Times don't add up");
