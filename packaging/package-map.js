...
exports.PackageMap = function (versions, options) {
  ...
  _.each(versions, function (version, packageName) {
    ...
    if (_isobuildCompilerJs.isIsobuildFeaturePackage(packageName)) {
      return;
    }
...
_.extend(exports.PackageMap.prototype, {
  ...
  getInfo: function (packageName) {
    var self = this;
    if (_.has(self._map, packageName)) return self._map[packageName];
    return null;
  },
  makeSubsetMap: function (packageNames) {
    ...
    _.each(packageNames, function (packageName) {
      ...
      if (!info) throw Error("not a subset: " + packageName);
  ...
  isSupersetOfJSON: function (mapJSON) {
    ...
    return _.all(mapJSON, function (jsonInfo, packageName) {
      ...
      if (!thisInfo) return false;
      if (jsonInfo.kind !== thisInfo.kind) return false;
...
exports.PackageMap.fromReleaseVersion = function (releaseVersion) {
  ...
  if (!toolPackageVersion) throw new Error("bad tool in release: " + releaseVersion.tool);
...
_.extend(exports.PackageMapDelta.prototype, {
  _storeAddOrChange: function (packageName, newInfo, oldVersion, anticipatedPrereleases, neededToUseUnanticipatedPrereleases) {
    ...
    if (newInfo.version === oldVersion) return;
