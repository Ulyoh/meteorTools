...
var _updateServerPackageData = function (dataStore, options) {
  var self = this;
  options = options || {};
  if (dataStore === null) throw Error("Data store expected");
...
var bundleSource = function (isopack, includeSources, packageDir) {
  ...
  _.each(includeSources, function (f) {
    files.copyFile(files.pathJoin(packageDir, f), files.pathJoin(sourcePackageDir, f));
  });
  ...
  if (files.exists(packageMapFilename)) throw Error(".versions file already exists? " + packageMapFilename);
  var pluginProviderPackageMap = isopack.pluginProviderPackageMap;
  if (!pluginProviderPackageMap) throw Error("no pluginProviderPackageMap on isopack?");
...
exports.publishPackage = function (options) {
  ...
  if (options['new'] && options.existingVersion) throw Error("is it new or does it exist?!?");
  ...
  if (!isopack) throw Error("no isopack " + name);
  ...
  if (!sourceFiles) throw Error("isopack doesn't know what its source files are?");
  ...
  if (packageSource.testName) {
    ...
    if (!testIsopack) throw Error("no testIsopack " + packageSource.testName);
    var testSourceFiles = testIsopack.getSourceFilesUnderSourceRoot(packageSource.sourceRoot);
    if (!testSourceFiles) throw Error("test isopack doesn't know what its source files are?");
