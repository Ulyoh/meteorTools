...
var defaultWarehouseDir = function () {
  ...
  if (process.env.METEOR_WAREHOUSE_DIR) return process.env.METEOR_WAREHOUSE_DIR;
...
exports._extractAndConvert = function (packageTarball, forceConvert) {
  ...
  if (process.platform === "win32" || forceConvert) {
    ...
    var _Isopack$readMetadataFromDirectory = Isopack.readMetadataFromDirectory(targetDirectory);

    var metadata = _Isopack$readMetadataFromDirectory.metadata;
    var originalVersion = _Isopack$readMetadataFromDirectory.originalVersion;
...
_.extend(exports.Tropohouse.prototype, {
  ...
  wipeAllPackages: function () {
    ...
    _.each(escapedPackages, function (packageEscaped) {
      ...
      _.each(versions, function (version) {
        ...
        if (/^[a-f0-9]{3,}$/.test(version)) return;
  ...
  installed: function (options) {
    var self = this;
    if (!options.packageName) throw Error("Missing required argument: packageName");
    if (!options.version) throw Error("Missing required argument: version");
  ...
  _alreadyDownloaded: function (options) {
    ...
    var _Isopack$readMetadataFromDirectory2 = Isopack.readMetadataFromDirectory(packagePath);

    var packageMetadata = _Isopack$readMetadataFromDirectory2.metadata;
  ...
  _makeDownloader: function (options) {
    ...
    if (!options.packageName) throw Error("Missing required argument: packageName");
    if (!options.version) throw Error("Missing required argument: version");
    if (!options.architectures) throw Error("Missing required argument: architectures");
      ...
      buildmessage.enterJob({
        ...
        _.each(buildsToDownload, function (build) {
          buildmessage.enterJob({
            ...
            try {
              var buildTempDir = self._downloadBuildToTempDir({ packageName: packageName, version: version }, build);
            } catch (e) {
              if (!(e instanceof files.OfflineError)) throw e;
        ...
        _.each(buildInputDirs, function (buildTempDir, i) {
          isopack._loadUnibuildsFromPath(packageName, buildTempDir, { firstIsopack: i === 0 });
        });
  ...
  downloadPackagesMissingFromMap: function (packageMap, options) {
    ...
    packageMap.eachPackage(function (packageName, info) {
      ...
      buildmessage.enterJob("checking for " + packageName + "@" + info.version, function () {
        ...
        if (downloader && downloaders) downloaders.push(downloader);
      });
    });
