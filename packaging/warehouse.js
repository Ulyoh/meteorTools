...
_.extend(warehouse, {
  ...
  getWarehouseDir: function () {
    ...
    if (process.env.METEOR_WAREHOUSE_DIR) return files.convertToStandardPath(process.env.METEOR_WAREHOUSE_DIR);
    ...
    // set)
    if (!files.usesWarehouse()) throw new Error("There's no warehouse in a git checkout");
  ...
  ensureReleaseExistsAndReturnManifest: function (release, quiet) {
    if (!files.usesWarehouse()) throw new Error("Not in a warehouse but requesting a manifest!");
  ...
  _calculateNewPiecesForRelease: function (releaseManifest) {
    ...
    if (newPieces.tools || !_.isEmpty(newPieces.packages)) return newPieces;
    return null;
  },
  ...
  _populateWarehouseForRelease: function (releaseVersion, showInstalling) {
    ...
    if (!releaseAlreadyExists) {
      ...
      if (process.env.METEOR_TEST_FAIL_RELEASE_DOWNLOAD === "offline") throw new files.OfflineError(new Error("scripted failure for tests"));
      if (process.env.METEOR_TEST_FAIL_RELEASE_DOWNLOAD === "not-found") throw new warehouse.NoSuchReleaseError();
    ...
    if (releaseAlreadyExists && !newPieces) return releaseManifest;
    ...
    if (!releaseAlreadyExists) {
      if (newPieces && newPieces.tools && newPieces.tools.needsDownload) {
        try {
          ...
          if (showInstalling) console.error("Failed to load tools for release " + releaseVersion);
          throw e;
        }
      ...
      _.each(newPieces && newPieces.packages, function (packageInfo, name) {
        if (packageInfo.needsDownload) packagesToDownload[name] = packageInfo.version;
      });
      if (!_.isEmpty(packagesToDownload)) {
        try {
          ...
          if (showInstalling) console.error("Failed to load packages for release " + releaseVersion);
          throw e;
        }
      }
  ...
  downloadToolsToWarehouse: function (toolsVersion, platform, warehouseDirectory, dontWriteFreshFile) {
    ...
    if (!dontWriteFreshFile) files.writeFile(warehouse.getToolsFreshFile(toolsVersion), '');
  },
  ...
  downloadPackagesToWarehouse: function (packagesToDownload, platform, warehouseDirectory, dontWriteFreshFile) {
    fiberHelpers.parallelEach(packagesToDownload, function (version, name) {
      ...
      if (!dontWriteFreshFile) files.writeFile(warehouse.getPackageFreshFile(name, version), '');
    });
  },
  ...
  _platform: function () {
    ...
    var arch = os.arch();
    if (arch === "ia32") arch = "i686";else if (arch === "x64") arch = "x86_64";else throw new Error("Unsupported architecture " + arch);
