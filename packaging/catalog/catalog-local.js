var _isobuildCompilerJs = require('../../isobuild/compiler.js');
...
_.extend(LocalCatalog.prototype, {
  toString: function () {
    var self = this;
    return "LocalCatalog [localPackageSearchDirs=" + self.localPackageSearchDirs + "]";
  },
  ...
  initialize: function (options) {
    ...
    self.localPackageSearchDirs = _.map(options.localPackageSearchDirs, function (p) {
      return files.pathResolve(p);
    });
    self.explicitlyAddedLocalPackageDirs = _.map(options.explicitlyAddedLocalPackageDirs, function (p) {
      return files.pathResolve(p);
    });
  ...
  getSortedVersionRecords: function (name) {
    ...
    if (_.has(_isobuildCompilerJs.KNOWN_ISOBUILD_FEATURE_PACKAGES, name)) {
      return _isobuildCompilerJs.KNOWN_ISOBUILD_FEATURE_PACKAGES[name].map(function (version) {
  ...
  _computeEffectiveLocalPackages: function () {
    ...
    buildmessage.enterJob("looking for packages", function () {
      _.each(self.explicitlyAddedLocalPackageDirs, function (explicitDir) {
        var packageJs = watch.readAndWatchFile(self.packageLocationWatchSet, files.pathJoin(explicitDir, 'package.js'));
      ...
      _.each(self.localPackageSearchDirs, function (searchDir) {
        ...
        _.each(possiblePackageDirs, function (subdir) {
          ...
          var packageJs = watch.readAndWatchFile(self.packageLocationWatchSet, files.pathJoin(absPackageDir, 'package.js'));
          if (packageJs !== null) {
