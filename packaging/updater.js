...
var checkForUpdate = function (showBanner, printErrors) {
  ...
  if (!release.current.isProperRelease()) return;
...
var updateMeteorToolSymlink = function (printErrors) {
  ...
  if (!latestRelease) throw Error("latest release doesn't exist?");
  if (!latestRelease.tool) throw Error("latest release doesn't have a tool?");
  ...
  if (!localLatestReleaseLink.startsWith(relativeToolPath + files.pathSep)) {
    ...
    if (!toolRecord) throw Error("latest release has no tool?");
